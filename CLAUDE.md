...
4. `memory/rules/map-creation-rules-v1.md` — the phase you're about to run — plus `memory/rules/rules-delta.md` for clarifications not yet folded into the main rules, and `memory/rules/prompt-interpretation-notes.md` if a prompt result is unclear.
...
A whole-column rename (like the Force matrix's third column already being renamed from Collateral to Trace) is a bigger edit than a single cell: record it in `memory/rules/rules-delta.md` first, then fold it into `map-creation-rules-v1.md` once adopted, per the normal rules-delta flow above.
...
- **Sanity-check a proposed move** — walk a card draw through the Awakening logic in `memory/rules/map-creation-rules-v1.md` before it's acted on physically.
...
- **Process the todo queue during a Maintenance result** — pick one item from `memory/tracking/map-todo.md` and complete, update, reschedule, retrigger, or move it into the next-session queue, per the Seed Matrix rule in `map-creation-rules-v1.md`.
...
- Adopt the planned Brown→Orange, Black→Purple color-channel rename once the user decides — must touch `map-creation-rules-v1.md`, `keyword-list.md`, and every past reference consistently, not just the rulebook.
